...
class OneMapClient():
	...
	def get_token(self, email=None, password=None):
		...
		response = requests.post(self.url_base + "/privateapi/auth/post/getToken",json=jsondata, headers={"Content-Type":"application/json"})
		response_data = json.loads(response.text)
...
if __name__ == "__main__":
	...
	while counter != df_length:
		...
		result = Client.search(get_data[counter])
		try:
			get_lat=result.get("results")[0]['LATITUDE']
			get_lon=result.get("results")[0]['LONGITUDE']
			print(f'\n\nLat= {get_lat}, Long= {get_lon}\n\n')
			with open('get_address.csv','a') as myfile:
				myfile.write(f'{get_data[counter]},{get_lat},{get_lon}\n')
			counter +=1
		except:
			print(f"No results found for {get_data[counter]}, skipping to the next row")
			with open('get_address.csv','a') as myfile:
